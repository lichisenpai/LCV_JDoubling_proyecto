...
def det_width (xx, yy):
    ...
    spline = UnivariateSpline(xx, yy-np.max(yy)/4, s=0)
...
yy, a, b = leer_archivo('SeñalesExperimentales/ha_06.slc')
...
pendiente = (yy[0]-yy[len(yy)-1])/(R2[0]-R1[0])
ordenada_o = pendiente*R1[0] - yy[R1[0]]

y = np.zeros(R2[0]-R1[0]-1)
for i in range(0, R2[0]-R1[0]-1):
    y[i] = yy[R1[0]+i] - (pendiente*i-ordenada_o)

print("Minimo: ", min(y))
y = y - min(y)
plt.figure(figsize=(20,10))
#plt.plot([iz, de], [yy[iz], yy[de]], color = "red")
...
intervalo = int(len(yy)/4)    #tiene los valores normales
m = 164
